...
<div class="page-content">
    ...
    <div class="card overflow-hidden">
        <x-ui.table-heading title="Daftar Pengguna">
            <a href="{{ route('users.create') }}" class="btn-primary btn-icon" title="Tambah User" aria-label="Tambah User">
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="table-actions">
                                <a href="{{ route('users.edit', $user) }}" class="btn-ghost btn-icon" title="Ubah" aria-label="Ubah {{ $user->name }}">
                                    <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('users.destroy', $user) }}" method="POST" class="delete-user-form">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn-ghost btn-icon text-danger hover:text-white hover:bg-danger" title="Hapus" aria-label="Hapus {{ $user->name }}">
                                        <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @else
                                <span class="inline-flex items-center justify-center btn-icon rounded-md text-secondary/70 cursor-default" title="Ini akun Anda, tidak bisa dihapus sendiri" aria-label="Ini akun Anda, tidak bisa dihapus sendiri">
                                    <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
