...
<div class="page-content">
    ...
    <x-ui.page-header title="Manajemen Lokasi" />

    <div class="card overflow-hidden">
        <x-ui.table-heading title="Daftar Lokasi">
            <a href="{{ route('locations.create') }}" class="btn-primary btn-icon" title="Tambah Lokasi" aria-label="Tambah Lokasi">
                        ...
                        <td>
                            <div class="table-actions">
                                <a href="{{ route('locations.edit', $location) }}"
                                   class="btn-ghost btn-icon" title="Ubah" aria-label="Ubah {{ $location->nama }}">
                                    <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('locations.destroy', $location) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn-ghost btn-icon text-danger hover:text-white hover:bg-danger" title="Hapus" aria-label="Hapus {{ $location->nama }}">
                                        <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
