    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="pagination-wrapper">
        ...
        <div class="flex items-center justify-between sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center px-4 py-2 text-sm font-normal text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-800 border border-[#E5E7EB] dark:border-gray-600 cursor-not-allowed">
                    {!! __('pagination.previous') !!}
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-4 py-2 text-sm font-normal text-[#111827] dark:text-gray-200 bg-white dark:bg-gray-800 border border-[#E5E7EB] dark:border-gray-600 hover:bg-[#F5F7FA] dark:hover:bg-gray-700 transition-colors">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-4 py-2 text-sm font-normal text-[#111827] dark:text-gray-200 bg-white dark:bg-gray-800 border border-[#E5E7EB] dark:border-gray-600 hover:bg-[#F5F7FA] dark:hover:bg-gray-700 transition-colors">
                    {!! __('pagination.next') !!}
                </a>
            @else
                <span class="inline-flex items-center px-4 py-2 text-sm font-normal text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-800 border border-[#E5E7EB] dark:border-gray-600 cursor-not-allowed">
            ...
            <div>
                <p class="text-sm text-gray-400 dark:text-gray-500">
                    ...
                    @if ($paginator->firstItem())
                        <span class="font-normal text-[#111827] dark:text-gray-200">{{ $paginator->firstItem() }}</span>
                        -
                        <span class="font-normal text-[#111827] dark:text-gray-200">{{ $paginator->lastItem() }}</span>
                    @else
                        {{ $paginator->count() }}
                    @endif
                    dari
                    <span class="font-normal text-[#111827] dark:text-gray-200">{{ $paginator->total() }}</span>
                    data
                </p>
            </div>
            <div>
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page" class="inline-flex items-center justify-center w-9 h-9 text-sm font-normal text-white bg-primary border border-primary dark:bg-primary-600 dark:border-primary-600">{{ $page }}</span>
                                @else
