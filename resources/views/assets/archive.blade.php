...
@section('title', __('ui.reports.arsip_title'))

@section('content')
<div class="page-content">

    <x-ui.page-header :title="__('ui.reports.arsip_title')" />

    {{-- Search & Filter --}}
    <div class="card overflow-hidden">
        <x-ui.table-heading :title="__('ui.reports.daftar_aset_diarsipkan')">
        ...
        <div class="card-body-compact border-b border-default">
            <form action="{{ route('assets.archive') }}" method="GET" class="filter-form">
                ...
                <a href="{{ route('assets.archive') }}" class="btn-ghost btn-icon" title="{{ __('ui.common.reset_filter') }}" aria-label="{{ __('ui.common.reset_filter') }}"><svg class="icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></a>
                @endif
            </form>
        </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>{{ __('ui.reports.kondisi') }}</th>
                        <th class="text-center">Status</th>
                        <th>{{ __('ui.reports.penghapusan') }}</th>
                ...
                <tbody>
                    ...
                    <tr class="cursor-pointer asset-row" data-url="{{ route('assets.show', $asset) }}">
                        ...
                        <td>
                            @if($disposalLog)
                                ...
                                <div class="text-xs text-secondary">{{ __('ui.reports.oleh') }} {{ $disposalLog->user?->name ?? __('ui.mutasi.sistem') }}</div>
                            @else
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-16">
                            <x-ui.empty-state
                                icon="package"
                                :title="__('ui.reports.empty_arsip_title')"
                                :description="__('ui.reports.empty_arsip_desc')" />
                        </td>
                    </tr>
                    @endforelse
                </tbody>
